...
'Access-Control-Allow-Headers':'Content-Type,X-Target,X-Cookies',
...
if(!t)return new Response(JSON.stringify({error:'Need X-Target header',usage:'Set X-Target header to the wizzair.com URL you want to proxy'}),{status:400,headers:{...cors,'Content-Type':'application/json'}});
let url;
try{url=new URL(t)}catch{return new Response(JSON.stringify({error:'Invalid URL in X-Target'}),{status:400,headers:{...cors,'Content-Type':'application/json'}})}
if(!url.hostname.endsWith('wizzair.com'))return new Response(JSON.stringify({error:'Only wizzair.com domains allowed'}),{status:403,headers:{...cors,'Content-Type':'application/json'}});
const h=new Headers();
...
}catch(e){return new Response(JSON.stringify({error:'Fetch failed: '+e.message}),{status:502,headers:{...cors,'Content-Type':'application/json'}})}
}
};
